...
#                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
    ...
    def write_to_csv(self, sTime, sDate):
        ...
        for i in range(P):
            df.loc[len(df.index)] = [i, self.classList[i][0], self.classList[i][1], self.classList[i][2],
                                        self.classList[i][3], self.classList[i][4], self.classList[i][5],
                                        self.classList[i][6], self.classList[i][7], sTime, eTime, sDate, eDate]
